###Figure 4 v3_3 (use the RNA Assay for Featureplots, maxcutoff q99)
...
a <- c('lightgrey', 'lightgrey', 'lightgrey', 'lightgrey', 'lightgrey','lightgrey', '#FB9902')
...
#mucin5AC
FeaturePlot(seurat_integrated, "HyS0004.446", order = TRUE, cols = c("lightgrey", "magenta"), 
...
#Chit1
FeaturePlot(seurat_integrated, "HyS0041.99", order = TRUE, cols = c("lightgrey", "yellow"), 
            min.cutoff = "q5", max.cutoff = "q99", slot = "data", pt.size = 2, label = FALSE) + labs(title = NULL) +
  theme(axis.text.x = element_text(face = "bold", size = 30, vjust = 0),
        axis.text.y = element_text(face = "bold", size = 30, hjust = 1),
        axis.title.y = element_text(face = "bold", size = 30),
